...
class PatchedInteraction(Interaction):
    ...
    async def deny(self, message: str) -> discord.message:
        return await self.response.send_message(embed=discord.Embed(description=f'{Emojis.DENY} {self.user.mention}: {message}', color=Colors.DENY), ephemeral=True)
    
    async def warn(self, message: str) -> discord.message:
        return await self.response.send_message(embed=discord.Embed(description=f'{Emojis.WARN} {self.user.mention}: {message}', color=Colors.WARN), ephemeral=True)
    
    async def approve(self, message: str, url: str=None) -> discord.Message:
        if url is None: return await self.response.send_message(embed=discord.Embed(description=f'{Emojis.APPROVE} {self.user.mention}: {message}', color=Colors.APPROVE), ephemeral=True)
        return await self.response.send_message(embed=discord.Embed(description=f'{Emojis.APPROVE} {self.user.mention}: {message}', color=Colors.APPROVE).set_image(url=url), ephemeral=True)
